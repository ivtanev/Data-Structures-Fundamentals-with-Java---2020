...
public class PriorityQueue<E extends Comparable<E>> implements AbstractQueue<E> {
    ...
    private void ensureNonEmpty() {
        if (this.size() == 0) {
            throw new IllegalStateException("Cannot peek empty priority queue");
        }
    }
    ...
    private void heapifyDown(int index) {
        while (getLeftChildIndex(index) < this.size() && isLess(index, this.getLeftChildIndex(index))) {
